...
PLOT_TS_BASIC_ALL         <- TRUE
PLOT_TS_SIGMA_ALL         <- TRUE
...
ICE_THRESH <- 0   # percent; if your data is fraction 0–1, set ICE_THRESH <- 0.15
...
for (sp in species_vec) {
  for (dkey in depth_keys) {
    ...
    if (PLOT_INTERACTIVE_3D) save_plotly_3d(df, sp, col_SST, col_SSS, col_O2, dkey, show_presence = TRUE)
    if (PLOT_STATIC_3D)      save_s3d_png(df, sp, col_SST, col_SSS, col_O2, dkey, show_presence = TRUE)
    if (PLOT_SEPARATION_3D)  save_sep3d_png(df, sp, col_SST, col_SSS, col_O2, dkey, show_presence = TRUE)
    if (PLOT_TS_BASIC)       save_TS_png(df, sp, col_SST, col_SSS, col_O2, dkey, with_sigma = FALSE, show_presence = TRUE)
    if (PLOT_TS_SIGMA)       save_TS_png(df, sp, col_SST, col_SSS, col_O2, dkey, with_sigma = TRUE,  show_presence = TRUE)
  }
}
...
for (dkey in depth_keys) {
  ...
  if (nrow(df0) == 0) { message("No data for ALL at ", depth_label(dkey)); next }
  
  if (PLOT_INTERACTIVE_3D_ALL) save_plotly_3d(df0, "ALL", col_SST, col_SSS, col_O2, dkey, show_presence = FALSE)
  if (PLOT_STATIC_3D_ALL)      save_s3d_png(df0, "ALL", col_SST, col_SSS, col_O2, dkey, show_presence = FALSE)
  if (PLOT_SEPARATION_3D_ALL)  save_sep3d_png(df0, "ALL", col_SST, col_SSS, col_O2, dkey, show_presence = FALSE)
  if (PLOT_TS_BASIC_ALL)       save_TS_png(df0, "ALL", col_SST, col_SSS, col_O2, dkey, with_sigma = FALSE, show_presence = FALSE)
  if (PLOT_TS_SIGMA_ALL)       save_TS_png(df0, "ALL", col_SST, col_SSS, col_O2, dkey, with_sigma = TRUE,  show_presence = FALSE)
}

# =======================================================
# ---- DEPTH PROFILES (per site & overlay)
# =======================================================
  ...
  if (PLOT_DEPTH_PROFILES_PER_SITE) {
    for (site in sites_keep) {
      ...
      p <- ggplot(interp, aes(x = value, y = depth, color = var)) +
        ...
        labs(
          title = paste("Depth Profiles —", name(site), "|", ice_caption()),
          x = NULL, y = "Depth (m)"
        ) +
        theme_bw(base_size = 12) +
    ...
    if (nrow(long_all) == 0) {
      ...
      for (v in c("temperature","salinity","o2")) {
        ...
        p <- ggplot(sub, aes(x = value, y = depth, color = Site)) +
          ...
          labs(
            title = paste0("Depth Profile — ", v, " | ", ice_caption()),
            x = NULL, y = "Depth (m)"
          ) +
          theme_bw(base_size = 12) +
...
# =======================================================
# ---- TIME–DEPTH (Hovmöller) SECTIONS (per site)
#      with per-site color limits and ice threshold
# =======================================================
if (MAKE_TIME_DEPTH_SECTIONS) {
  ...
  for (site in sites_keep) {
    ...
    for (v in c("temperature", "salinity", "o2")) {
      ...
      plots[[v]] <- hov_plot(
        grid,
        paste0(name(site), " — ", var_labels[[v]], " | ", ice_caption()),
        var_labels[[v]],
        lims = v_minmax
      )
    }

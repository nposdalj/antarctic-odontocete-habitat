...
- **DeseasoningMethods.R:** Uses ten years of Copernicus data (stored in the "Deseasoning" folder of "Copernicus") to explore different deseasoning and anomalizing methods, generating timeseries to compare methods. Compares STL deseasoning and anomalies with unchanged data (for chlorophyll-a, net primary production, sea surface temperature, sea surface salinity, and mixed layer depth) in single-variable GAMs to see how different methods of incorporating the same data affect modeling.

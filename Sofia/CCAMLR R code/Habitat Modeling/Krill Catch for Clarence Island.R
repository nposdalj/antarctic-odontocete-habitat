...
center_point <- st_sfc(st_point(c(-54.05, -61.2)), crs = 4326)


plot_area <- st_buffer(st_transform(center_point, 3031), dist = set_units(100000, m)) %>%
  st_transform(4326)
...
islands <- data.frame(name = "Clarence Island", lon = -54.05, lat = -61.2)
...
ggplot() +
  ...
  labs(
    title = "Krill Catch around Clarence Island",

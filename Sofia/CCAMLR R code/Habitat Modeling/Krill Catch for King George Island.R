...
king_george <- st_sfc(st_point(c(-58.0, -62.0)), crs = 4326)


plot_area <- st_buffer(st_transform(king_george, 3031), dist = set_units(100000, m)) %>%
  st_transform(4326)
...
bathy <- getNOAA.bathy(
  lon1 = st_bbox(plot_area)["xmin"] - 0.1,
  lon2 = st_bbox(plot_area)["xmax"] + 0.1,
  lat1 = st_bbox(plot_area)["ymin"] - 0.1,
  lat2 = st_bbox(plot_area)["ymax"] + 0.1,
  resolution = 1
)
bathy_df <- fortify.bathy(bathy)


islands <- data.frame(name = "King George Island", lon = -58.0, lat = -62.0)
...
ggplot() +
  ...
  geom_text(data = islands, aes(x = lon, y = lat, label = name),
            nudge_y = 0.02, fontface = "bold", color = "red", size = 4) +
  
  annotation_north_arrow(location = "tr", which_north = "true", style = north_arrow_fancy_orienteering) +
  annotation_scale(location = "bl", width_hint = 0.4) +
  
  coord_sf(xlim = c(st_bbox(plot_area)["xmin"], st_bbox(plot_area)["xmax"]),
           ylim = c(st_bbox(plot_area)["ymin"], st_bbox(plot_area)["ymax"])) +
  
  labs(
    title = "Krill Catch around King George Island",
...
ggplot() +
  ...
  theme(
    ...
    plot.title = element_text(size = 18, face = "bold"),
    plot.subtitle = element_text(size = 12),
    panel.grid = element_line(color = "t784ransparent")
  )
...
ggplot() +
  ...
  scale_size_continuous(range = c(1, 8), name = "Catch (kg)") +

...
URL = 'http://tds.hycom.org/thredds/dodsC/GLBv0.08/expt_56.3?'; % Make sure this matches URL in line 158
...
%% Clarence Island
clc;clearvars
aimpath = "C:/Users/HARP/Documents/GitHub/antarctic-odontocete-habitat/Environmental Data/HYCOM/CI";
region = [306.016567 307.016567 -61.751867 -60.751867]; % +/- 0.5 degrees from CI
timeList = datetime(2016,2,4):datetime(2016,10,1);
varList = {'ssh','temp','salt','u','v'};
...
aimpath = "C:/Users/HARP/Documents/GitHub/antarctic-odontocete-habitat/Environmental Data/HYCOM/CI";
region = [306.016567 307.016567 -61.751867 -60.751867]; % +/- 0.5 degrees from CI
timeList = datetime(2016,10,2):datetime(2016,12,2);
varList = {'ssh','temp','salt','u','v'};
...
%% Elephant Island
% This site is outside the temporal range of the data source for KGI and CI
% So, need to use a different URL, and make sure that the URL in the
% function is changed to match.
clc;clearvars
...
varList = {'ssh','temp','salt','u','v'};
URL = 'http://tds.hycom.org/thredds/dodsC/GLBv0.08/expt_53.X?'; % Change URL in line 175 to match this
...
%% Clarence Island Island LONG
clc;clearvars
aimpath = "D:/HYCOM long";
region = [306.016567 307.016567 -61.751867 -60.751867]; % +/- 0.5 degrees from CI
timeList = datetime(2014,7,1):datetime(2016,4,30);
varList = {'ssh','temp','salt','u','v'};
URL = 'http://tds.hycom.org/thredds/dodsC/GLBv0.08/expt_56.3?'; % Make sure this matches URL in line 158
...
if strcmp(URL,'http://tds.hycom.org/thredds/dodsC/GLBv0.08/expt_53.X?') % http://tds.hycom.org/thredds/dodsC/GLBy0.08/expt_93.0?
    varBase = {'ssh','temp','salt','u','v'};
    stdBase = {'surf_el', 'water_temp','salinity','water_u','water_v'};
else
    varBase = {'mixed_layer_thickness','ssh','ubar','vbar','surface_boundary_layer_thickness'};
    stdBase = {'mixed_layer_thickness','ssh','u_barotropic_velocity','v_barotropic_velocity','surface_boundary_layer_thickness'};
end
...
ind_vars = cellfun(@(x) find(contains(varBase, x), 1, 'first'), varList); % edited here
...
% else
    ...
    if any(strcmp({info.Variables.Name}, 'depth'))
    ...
    % Get variable dimension info
    varDims = info.Variables(strcmp({info.Variables.Name}, stdName)).Dimensions;
    nDims = numel(varDims);

    if nDims == 4
        % 4D variable: [lon, lat, depth, time]
        varData = squeeze(ncread(URL, stdName, ...
            [min(indLons),min(indLats),1,indTime], ...
            [abs(diff(indLons))+1,abs(diff(indLats))+1,nLayers,1]));
    elseif nDims == 3
        % 3D variable: [lon, lat, time]
        varData = squeeze(ncread(URL, stdName, ...
            [min(indLons),min(indLats),indTime], ...
            [abs(diff(indLons))+1,abs(diff(indLats))+1,1]));
    else
        error(['Unexpected number of dimensions (', num2str(nDims), ') for variable ', stdName]);
    end

    D.(varName) = varData;
    clear varData
%end
    %commented out to hopefully debug
%    for iVar = 1:nVars
 %       stdName = stdList{iVar};
 %       varName = varList{iVar};
  %      if any(strcmp({info.Variables.Name}, 'depth'))

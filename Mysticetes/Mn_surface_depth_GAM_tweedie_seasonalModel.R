...
species <- c('Mn') # options: BW29, BW37, Oo, Pm, Gm, Mn
...
BlockMod_CI <- gam(BW37 ~ s(ice_conc,k=4),
...
binned_plot <- grid.arrange(binnedTimeseries(EI_binned,'EI',ACFval_EI), binnedTimeseries(KGI_binned,'KGI',ACFval_KGI), 
...
#Full model
EI_gam <- gam(BW37 ~ s(ice_conc,k=3,sp=0.1) + s(ice_diff,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")

# Remove ice_diff
EI_final <- gam(BW37 ~ s(ice_conc,k=3),
                family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")
...
# Full GAM
KGI_gam <- gam(BW37 ~ s(ice_conc,k=4,sp=0.1) + s(ice_diff,k=4,sp=0.1),
               family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")

# Remove ice_diff
KGI_final <- gam(BW37 ~ s(ice_conc,k=4,sp=0.1),
                 family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")


# --------------------- Step 5c: Clarence Island GAM ------------------------------------
# Final model
CI_gam <- gam(BW37 ~ s(ice_conc,k=3,sp=0.1) + s(ice_diff,k=4,sp=0.1) + s(ice_thickness,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9),  data = CI_binned, method = "REML")

# Remove ice_diff
CI_gam <- gam(BW37 ~ s(ice_conc,k=3,sp=0.1) + s(ice_thickness,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9),  data = CI_binned, method = "REML")

CI_final <- gam(BW37 ~ s(ice_thickness,k=3),
              family = tw(link = "log", a = 1.1, b = 1.9),  data = CI_binned, method = "REML")
...
visualizeGAM <- function(gam, predictors, sp) {
  ...
  final_plot <- wrap_plots(all_plots, nrow = row, ncol = col, guides = "collect") &
    plot_annotation(title = paste0("BW37 at ",
...
CI_pred <- c('ice_thickness')
CI_plots <- visualizeGAM(CI_final, CI_pred, 'KGI')

EI_pred <- c('ice_conc')
EI_plots <- visualizeGAM(EI_final, EI_pred, 'KGI')

...
dailyDetection <- read.csv("/Users/Default User.DESKTOP-VFJ1NAJ/Desktop/Baleen Whalesssss/mysticetes/Antarc_Mysticetes_daily_5min_bins.csv")
...
sites <- as.vector(c('CI')) # options: EI, KGI, CI. For EI change y lim to 600. 
...
aggregatePlot <- function(data, vars, plot_species, depths,site) {
  ...
  if(plot_species == 'none') 
    ...
    # Adding timeseries for species presence to list of plots !!!!!!!!YLIM!!!!!!!!!! 1460 except for EI (600)
    species_ts <- ggplot(data = data, mapping = aes(x = date, y = .data[[plot_species]])) + 
      ...
      labs(y = NULL, x = NULL, title = paste('Minutes Present per Day')) + ylim(0,1460) +

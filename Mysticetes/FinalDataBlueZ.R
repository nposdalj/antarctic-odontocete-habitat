...
dailyDetection <- read.csv("/Users/Default User.DESKTOP-VFJ1NAJ/Desktop/Baleen Whalesssss/mysticetes/Antarc_Mysticetes_daily_hour_bins.csv")
...
sites <- as.vector(c('EI')) # options: EI, KGI, CI
...
aggregatePlot <- function(data, vars, plot_species, depths,site) {
  ...
  if(plot_species == 'none') 
    ...
    species_ts <- ggplot(data = data, mapping = aes(x = date, y = .data[[plot_species]])) + 
      ...
      labs(y = NULL, x = NULL, title = paste(' Hours Present per Day')) + ylim(0,27) +
    ...
    #!!!!!!!!!!!!PLOTTITLE!!!!!!!!!!!!!!!!!!!!!!!!
    title <- paste0(name(plot_species)," at ", name(site), " Surface")
  }

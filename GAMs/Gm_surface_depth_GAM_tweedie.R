...
#....several steps later
EI_pred <- c("FSLE", "SSH", "mixed_layer", "ice_conc", 'fsle_orient', "EKE_mad_0",'salinity_16','EKE_635','EKE_16','o2_635')
mod_formula <- paste(species, "~", paste(EI_pred, collapse = " + "))
EI_vif <- glm(as.formula(mod_formula), data = EI_binned)
vif(EI_vif)
# FSLE         SSH mixed_layer    ice_conc fsle_orient   EKE_mad_0 salinity_16     EKE_635      EKE_16      o2_635 
# 2.463109    1.414458    3.138762    3.583536    2.524764    1.778021    3.360498    3.169596    2.494627    4.145326 
...
vif(KGI_vif)
#...several steps later
KGI_pred <- c("FSLE", "SSH", "mixed_layer", "ice_conc", 'fsle_orient', "EKE_mad_0",
              +               'salinity_635','EKE_635','EKE_16','o2_635','o2_16','productivity_16')
mod_formula <- reformulate(KGI_pred, response = species)
KGI_vif <- lm(mod_formula, data = KGI_binned)
vif(KGI_vif)
# FSLE             SSH     mixed_layer        ice_conc     fsle_orient       EKE_mad_0    salinity_635         EKE_635 
# 1.661651        4.838259        2.543094        5.798132        1.263310        1.501214        2.691611        2.180429 
# EKE_16          o2_635           o2_16 productivity_16 
# 2.012724        1.514724        2.056125        3.053942 
...
CI_vif <- lm(mod_formula, data = CI_binned)
vif(CI_vif)
# FSLE            SSH    mixed_layer       ice_conc    fsle_orient      EKE_mad_0 productivity_0   salinity_635 
# 4.011418       2.180992       2.027452       2.575802       1.256336       2.201345       6.277951       4.447558 
# EKE_635         EKE_16         o2_635          o2_16        chla_16 
# 1.667425       1.394211       3.142770       5.184223       5.528347 
...
EI_gam <- gam(Gm ~ s(FSLE,k=4) + s(mixed_layer,k=4,sp=0.1) + s(SSH,k=4,sp=0.1) +
                s(EKE_mad_0,k=4,sp=0.1) + s(ice_conc,k=4) + s(salinity_16,k=4,sp=0.1) + s(EKE_635,k=4,sp=0.1) +
                s(EKE_16,k=4,sp=0.1) + s(o2_635,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")

# Remove EKE
EI_gam <- gam(Gm ~ s(FSLE,k=4) + s(mixed_layer,k=4,sp=0.1) + s(SSH,k=4,sp=0.1) +
                s(ice_conc,k=4) + s(salinity_16,k=4,sp=0.1) + s(EKE_635,k=4,sp=0.1) +
                s(EKE_16,k=4,sp=0.1) + s(o2_635,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")

# Remove SSH
EI_gam <- gam(Gm ~ s(FSLE,k=4) + s(mixed_layer,k=4,sp=0.1) + 
                s(ice_conc,k=4) + s(salinity_16,k=4,sp=0.1) + s(EKE_635,k=4,sp=0.1) +
                s(EKE_16,k=4,sp=0.1) + s(o2_635,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")
...
#Remove FSLE
EI_final <- gam(Gm ~ s(mixed_layer,k=4,sp=0.1) + s(salinity_16,k=4,sp=0.1) + s(EKE_16,k=4,sp=0.1) + s(o2_635,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")
...
KGI_gam <- gam(Gm ~ s(FSLE,k=4) + s(SSH,k=4) + s(mixed_layer,k=4) + s(ice_conc,k=4) + 
                 ...
                 s(o2_635,k=4) +s(o2_16, k =4) + s(productivity_16, k = 4), 
...
KGI_gam <- gam(Gm ~ s(FSLE,k=4) + s(SSH,k=4) + s(mixed_layer,k=4) + s(ice_conc,k=4) + 
                 s(EKE_mad_0,k=4) + s(salinity_635,k=4) + s(EKE_635,k=4) + 
                 s(o2_16, k =4) + s(productivity_16, k = 4), 
               family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")

# Remove ML
KGI_gam <- gam(Gm ~ s(FSLE,k=4) + s(SSH,k=4) + s(ice_conc,k=4) + 
                 s(EKE_mad_0,k=4) + s(salinity_635,k=4) + s(EKE_635,k=4) + 
                 s(o2_16, k =4) + s(productivity_16, k = 4), 
               family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")
...
# Remove o2_16
KGI_gam <- gam(Gm ~ s(FSLE,k=4) + s(SSH,k=4) + s(ice_conc,k=4) + 
                 s(salinity_635,k=4) + s(EKE_635,k=4) + s(productivity_16, k = 4), 
               family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")

# Remove salinity_635
KGI_gam <- gam(Gm ~ s(FSLE,k=4) + s(SSH,k=4) + s(ice_conc,k=4) + 
                 s(EKE_635,k=4) + s(productivity_16, k = 4), 
               family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")

# Remove FSLE
KGI_final <- gam(Gm ~ s(SSH,k=4) + s(ice_conc,k=4) + 
                 s(EKE_635,k=4) + s(productivity_16, k = 4), 
               family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")
...
CI_gam <- gam(Gm ~ s(FSLE,k=4) + s(SSH,k=4) + s(mixed_layer,k=4) +
                s(ice_conc,k=4) + s(EKE_mad_0,k=4) + s(productivity_0,k=4) +
                s(salinity_635,k=4) + s(EKE_635,k=4) + s(EKE_16,k=4) + s(o2_635,k=4)+
                s(o2_16,k=4) + s(chla_16,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")

# Remove productivity_0
CI_gam <- gam(Gm ~ s(FSLE,k=4) + s(SSH,k=4) + s(mixed_layer,k=4) +
                s(ice_conc,k=4) + s(EKE_mad_0,k=4) +
                s(salinity_635,k=4) + s(EKE_635,k=4) + s(EKE_16,k=4) + s(o2_635,k=4)+
                s(o2_16,k=4) + s(chla_16,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")
...
CI_gam <- gam(Gm ~ s(FSLE,k=4) + s(SSH,k=4) + s(mixed_layer,k=4) +
                s(ice_conc,k=4) + s(EKE_mad_0,k=4) +
                s(salinity_635,k=4) + s(EKE_635,k=4) + s(o2_635,k=4)+
                s(o2_16,k=4) + s(chla_16,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")
...
# Remove ML
CI_gam <- gam(Gm ~ s(ice_conc,k=4) + s(EKE_mad_0,k=4) +
                s(EKE_635,k=4) + s(o2_635,k=4)+
                s(o2_16,k=4) + s(chla_16,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")
...
# Remove o2_16
CI_gam <- gam(Gm ~ s(ice_conc,k=4) + s(EKE_mad_0,k=4) +
                s(EKE_635,k=4) + s(o2_635,k=4) + s(chla_16,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")

# Remove ice
CI_gam <- gam(Gm ~ s(EKE_mad_0,k=4) +
                s(EKE_635,k=4) + s(o2_635,k=4) + s(chla_16,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")

# Remove EKE_mad_0
CI_final <- gam(Gm ~ s(EKE_635,k=4) + s(o2_635,k=4) + s(chla_16,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9), data = KGI_binned, method = "REML")
...
KGI_pred <- c('SSH','productivity_16','EKE_635','ice_conc')
KGI_plots <- visualizeGAM(KGI_final, KGI_pred, 'KGI')

EI_pred <- c('mixed_layer','salinity_16','EKE_16','o2_635')
EI_plots <- visualizeGAM(EI_final, EI_pred, 'EI')


CI_pred <- c('EKE_635','o2_635','chla_16')

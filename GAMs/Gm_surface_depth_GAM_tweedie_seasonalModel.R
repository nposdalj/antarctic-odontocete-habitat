...
allData_EI <- allData %>% 
  filter(Site == "EI")
BlockMod_EI <- gam(Gm ~ s(julian_day,k=4) + s(ice_conc,k=4),
...
allData_KGI <- allData %>% 
  filter(Site == "KGI")
BlockMod_KGI <- gam(Gm ~ s(julian_day,k=4) + s(ice_conc,k=4),
...
allData_CI <- allData %>% 
  filter(Site == "CI")
BlockMod_CI <- gam(Gm ~ s(julian_day,k=4) + s(ice_conc,k=4),
...
# ELEPHANT ISLAND
EI_pred <- c('julian_day','AAO','ice_conc', 'ice_diff', 'ice_thickness','ice_regime')
...
#....several steps later
EI_pred <- c('julian_day','AAO','ice_conc', 'ice_diff')
mod_formula <- paste(species, "~", paste(EI_pred, collapse = " + "))
# EI vif model is not converging when run as a logistic regression, so running as a linear regression instead
EI_vif <- glm(as.formula(mod_formula), data = EI_binned)
vif(EI_vif)
# julian_day        AAO   ice_conc   ice_diff 
# 2.324346   1.384173   2.855349   1.034767 

# KING GEORGE ISLAND
KGI_pred <- c('julian_day','AAO','ice_conc', 'ice_diff', 'ice_thickness','ice_regime')
mod_formula <- reformulate(KGI_pred, response = species)
KGI_vif <- lm(mod_formula, data = KGI_binned)
vif(KGI_vif)
#...several steps later
KGI_pred <- c('julian_day','AAO','ice_conc', 'ice_diff')
mod_formula <- reformulate(KGI_pred, response = species)
KGI_vif <- lm(mod_formula, data = KGI_binned)
vif(KGI_vif)
# julian_day        AAO   ice_conc   ice_diff 
# 1.289099   1.130160   1.175503   1.052529 

# CLARENCE ISLAND
CI_pred <- c('julian_day','AAO','ice_conc', 'ice_diff', 'ice_thickness','ice_regime')
mod_formula <- reformulate(CI_pred, response = species)
CI_vif <- lm(mod_formula, data = CI_binned)
vif(CI_vif)

#...several steps later
CI_pred <- c('julian_day','AAO','ice_conc', 'ice_diff', 'ice_thickness')
mod_formula <- reformulate(CI_pred, response = species)
CI_vif <- lm(mod_formula, data = CI_binned)
vif(CI_vif)
# julian_day           AAO      ice_conc      ice_diff ice_thickness 
# 1.172966      1.238549      3.349454      1.112216      3.323087 
...
#Full model
EI_gam <- gam(Gm ~ s(julian_day,k=4) + s(ice_conc,k=4,sp=0.1) + s(AAO,k=4,sp=0.1) +
                s(ice_diff,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")

# Remove AAO
EI_gam <- gam(Gm ~ s(julian_day,k=4) + s(ice_conc,k=4,sp=0.1) + s(ice_diff,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")

# Remove ice_diff
EI_gam <- gam(Gm ~ s(julian_day,k=4) + s(ice_conc,k=4,sp=0.1),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")

# Remove ice_conc
EI_final <- gam(Gm ~ s(julian_day,k=4),
              family = tw(link = "log", a = 1.1, b = 1.9), data = EI_binned, method = "REML")


# -------------------- Step 5b: King George Island GAM -------------------------
# Full GAM
KGI_gam <- gam(Gm ~ s(julian_day,k=4) + s(ice_conc,k=4,sp=0.1) + s(AAO,k=4,sp=0.1) +
                s(ice_diff,k=4,sp=0.1),
...
# Final model
CI_gam <- gam(Gm ~ s(julian_day, bs = "cc",k=4) + s(ice_conc,k=4,sp=0.1) + s(AAO,k=4,sp=0.1) +
                 s(ice_diff,k=4,sp=0.1) + ice_regime,
               family = tw(link = "log", a = 1.1, b = 1.9), data = CI_binned, method = "REML")

# Remove AAO
CI_final <- gam(Gm ~ s(julian_day,bs = "cc",k=4) + s(ice_conc,k=4,sp=0.1) + 
                s(ice_diff,k=4,sp=0.1) + ice_regime,
              family = tw(link = "log", a = 1.1, b = 1.9), data = CI_binned, method = "REML")
...
CI_pred <- c('julian_day','ice_diff','ice_conc')

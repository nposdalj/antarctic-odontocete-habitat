# Old modeling script: using all variables (across depths) at +/- 0.5 degrees from site
# Started modeling KGI, ran into issues with significance (sea ice concentration dropped out of model)
# GAM had edf of 1 for most variables, which doesn't make sense because relationships should be nonlinear
# If rerunning this code, load allData_hfdeg.csv

# Next steps: 40 km bounding box for environmental variables, dropping all variables at depth,
#   dropping AAO, dropping julian day (except for at KGI), dropping ice thickness (all before running VIF)
